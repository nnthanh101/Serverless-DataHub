...
# export Job4UByod_CLONE_URL=$(aws codecommit get-repository --repository-name ${Job4UByodRepo} --output json | jq '.repositoryMetadata.cloneUrlHttp' | sed "s/\"/ /g" ) 
